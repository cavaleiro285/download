...
if (!isset($url) || $url == '') {
    $valid = 0;
    $failure .= "O URL do vídeo deve ser inserido!
<br/>";
} else {
        ...
        $valid = 0;
        $failure .= "O URL do vídeo do YouTube é inválido
!<br/>";
        }
        $yt->get_url();
        if ($yt->error == TRUE) {
        $valid = 0;
        $failure .= "O sistema não conseguiu obter links de download, por favor, tente novamente.<br/>";
        }
}
if($valid == 1){
...
<div id="myModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Error de Link</h4>
      </div>
      <div class="modal-body">
